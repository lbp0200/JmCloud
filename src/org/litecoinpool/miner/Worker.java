...
public class Worker extends Observable implements Runnable {
	...
	public static enum Notification {
		SYSTEM_ERROR, PERMISSION_ERROR, CONNECTION_ERROR, AUTHENTICATION_ERROR, COMMUNICATION_ERROR, LONG_POLLING_FAILED, LONG_POLLING_ENABLED, NEW_BLOCK_DETECTED, NEW_WORK, POW_TRUE, POW_FALSE, TERMINATED
	};
						...
						if (lpUrl == null) {
							try {
								if ((lpUrl = curWork.getLongPollingURL()) != null) {
									(threads[0] = new Thread(new LongPoller()))
											.start();
									setChanged();
				...
				while (running) {
					try {
						if (curWork.meetsTarget(nonce, hasher)) {
							new Thread(new WorkSubmitter(curWork, nonce))
									.start();
							if (lpUrl == null) {
								synchronized (Worker.this) {
									curWork = null;
						...
						if (throttleFactor > 0.0
							...
							LockSupport.parkNanos(Math.max(0L,
									(long) (throttleFactor * dt)));
							t0 = System.nanoTime();
						}
					} catch (NullPointerException e) {
						try {
	...
	private class WorkSubmitter implements Runnable {
		...
		public void run() {
			try {
				...
				setChanged();
				notifyObservers(result ? Notification.POW_TRUE
						: Notification.POW_FALSE);
			} catch (IOException e) {
			}
		}
	}
