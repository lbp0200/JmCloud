...
public class Work {
	...
	private static final Pattern dataPattern = Pattern
			.compile("\"data\"\\s*:\\s*\"([0-9a-f]+)\"");
	private static final Pattern targetPattern = Pattern
			.compile("\"target\"\\s*:\\s*\"([0-9a-f]+)\"");
	private static final Pattern resultPattern = Pattern
			.compile("\"result\"\\s*:\\s*([0-9A-Za-z]+)");
	...
	public boolean meetsTarget(int nonce, Hasher hasher)
	...
	public static HttpURLConnection getJsonRpcConnection(URL url,
			String request, String auth) throws IOException {
		return getJsonRpcConnection((HttpURLConnection) url.openConnection(),
				request, auth);
	}

	public static HttpURLConnection getJsonRpcConnection(
			HttpURLConnection conn, String request, String auth)
			throws IOException {
	...
	public static String getConnectionContent(HttpURLConnection conn)
			throws IOException {
