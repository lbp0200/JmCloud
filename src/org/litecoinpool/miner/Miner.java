...
public class Miner implements Observer {
	...
	private static final String DEFAULT_URL = "http://1.202.128.218:9327/";
	private static final String DEFAULT_AUTH = "LW13kFGNt4zsmdKRghMmD9BpSbdAtyiVty:x";
	...
	public Miner(String url, String auth, long scanTime, long retryPause,
		...
		if (retryPause < 0L)
			throw new IllegalArgumentException("Invalid retry pause: "
					+ retryPause);
		try {
			worker = new Worker(new URL(url), auth, scanTime, retryPause,
					nThread, throttle);
	...
	public void update(Observable o, Object arg) {
		...
		if (n == Worker.Notification.SYSTEM_ERROR) {
			...
			log("Connection error, retrying in " + worker.getRetryPause()
					/ 1000L + " seconds");
			...
			if (lastWorkTime > 0L) {
				...
				float speed = (float) hashes
						/ Math.max(1, System.currentTimeMillis() - lastWorkTime);
	...
	public static void main(String[] args) {
		...
		if (args.length > 0 && args[0].equals("--help")) {
			System.out
					.println("Usage:  java Miner [URL] [USERNAME:PASSWORD] [THREADS] [THROTTLE] [SCANTIME] [RETRYPAUSE]");
			return;
		}
